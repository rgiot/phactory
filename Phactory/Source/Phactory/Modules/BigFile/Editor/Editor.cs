...
namespace CPCBigFile.Controller
{
    ...
    public class Editor : PhactoryHost.EditorPlugin
    {
        ...
        {
            return "BigFile Editor";
        }

        public string GetDescription()
        {
            return "Many files merged into a single big one";
        }
        ...
        {
            Document.Document newDoc = new Document.Document();

            return Host.XMLWrite(Host.GetFileInfo(resource).FullName, newDoc );
        }
        ...
        public void OpenResource(Panel parentPanel, PhactoryHost.Database.Resource resource)
        {
            ...
            {
                resourceBinding.View.Document.Expand();
        ...
        public void SaveResource(PhactoryHost.Database.Resource resource)
        {
            ...
            if (writeOK)
            {
                ...
                foreach (Document.File file in viewBinding[resource].View.Document.Files)
                {
                    dependentResources.Add(Plugin.ControllerEditor.Host.GetResource(file.ResourceID));
                }
