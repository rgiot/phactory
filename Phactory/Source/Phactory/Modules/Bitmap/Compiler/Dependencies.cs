        ...
        public List<int> GetBrokenResourceIDs(PhactoryHost.Database.Resource resource)
        {
            ...
            if (document is Document.Document)
            {
                foreach (Document.Item item in document.Items)
                {
                    ...
                    {
                        if (Host.GetResource(item.MaskResourceID) == null)
                        {
                            brokenResourceIDs.Add(item.MaskResourceID);
                        }
                    }
                }
            }
        ...
        public void UpdateDependencies(PhactoryHost.Database.Resource resource)
        {
            ...
            if (document is Document.Document)
            {
                foreach (Document.Item item in document.Items)
                {
                    ...
                    if (item.UseMaskType == Document.UseMaskType.BitmapMask)
                    {
                        PhactoryHost.Database.Resource depMaskResource = Host.GetResource(item.MaskResourceID);
                        if (depMaskResource != null)
                        {
                            dependentResources.Add(depMaskResource);
                        }
                    }

                    if ((item.IsMerge) && (item.UseMaskType == UseMaskType.ColorMask) &&
                        (item.MaskMergeResourceID != -1))
                    {
                        PhactoryHost.Database.Resource depMaskResource = Host.GetResource(item.MaskMergeResourceID);
                        if (depMaskResource != null)
                        {
                            dependentResources.Add(depMaskResource);
                        }
                    ...
                    {
                        PhactoryHost.Database.Resource depMaskResource = Host.GetResource(item.CloudResourceID);
                        if (depMaskResource != null)
                        {
                            dependentResources.Add(depMaskResource);
                        }
