...
SDLKey GLIntro::GetKey()
{
	return SDLK_1;
}

const char *GLIntro::GetName()
{
	return "GLIntro";
}
...
const int logoWidth = 384/2;
const int logoX = ((384/2)-logoWidth)/2;
const int logoY = (205-logoHeight)/2;
...
static void DoScreenshot()
{
	...
	float screenHeight = 205;
        ...
        sprintf(screenshotName, "IntroRotateLogo%02d.bmp", screenshotCounter);
		screenshotCounter++;
...
void GLIntro::Draw()
{
	...
	glTexCoord2f (1.0, 1.0);
      glVertex3f(  w,  h,  0.0f ); /* Top Right Of The Quad    */
	  
		glTexCoord2f (0.0, 1.0);
      glVertex3f( -w,  h,  0.0f ); /* Top Left Of The Quad     */
	  
		glTexCoord2f (0.0, 0.0);
      glVertex3f( -w, -h,  0.0f ); /* Bottom Left Of The Quad  */
	  
		glTexCoord2f (1.0, 0.0);
      glVertex3f(  w, -h,  0.0f ); /* Bottom Right Of The Quad */
    glEnd( );   
skip2:

    glLoadIdentity( );
//    glTranslatef( sin(rquad*0.01f)*2.0f, 0.0f, -8.0f );
    ...
    glColor3f( 1, 1, 1);
	// glColor3f( 1, 1, 1); WHEN USING TEXTURE AS BACKGROUND
	...
	/*float ratio = 0.011f;
	float w2 = (384.0f/2)*ratio;
	float h2 = 88.0f*ratio;
	*/

	float ratio = 0.02f;
	float w2 = (256.0/1.24)*ratio;
	float h2 = 206.9f*ratio;


	glTexCoord2f (1.0, 1.0);
      glVertex3f(  w2,  h2,  0.0f ); /* Top Right Of The Quad    */
	  
		glTexCoord2f (0.0, 1.0);
      glVertex3f( -w2,  h2,  0.0f ); /* Top Left Of The Quad     */
	  
		glTexCoord2f (0.0, 0.0);
      glVertex3f( -w2, -h2,  0.0f ); /* Bottom Left Of The Quad  */
	  
		glTexCoord2f (1.0, 0.0);
      glVertex3f(  w2, -h2,  0.0f ); /* Bottom Right Of The Quad */
