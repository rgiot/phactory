...
//	fs->root_entry_count = GET_16BIT_WORD(fs->currentsector.sector, BPB_ROOTENTCNT);
	...
	{
          fs->cluster_begin_lba=fs->cluster_begin_lba+fs->fat_sectors;
	}
	...
	while (TRUE)
	{
		...
		if (fatfs_sector_reader(fs, Cluster, x++, FALSE)) // If sector read was successfull
		{
			...
			for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
			{
				...
				{
					z80_memset(short_filename, 0, sizeof(short_filename));
					...
					for (i=8; i<11; i++)
					{
						...
						if (directoryEntry->Name[i] != (unsigned char)' ')
							dotRequired = 1;
					}
		...
		if (fatfs_sector_reader(fs, dirls->cluster, dirls->sector, FALSE))
		{
			...
			for (item = dirls->offset; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
			{
				...
				else if (fatfs_entry_lfn_exists(&lfn, directoryEntry) )
				{
					// Get text
					long_filename = fatfs_lfn_cache_get(&lfn);
					fat_strncpy(entry->filename, long_filename, FATFS_MAX_LONG_FILENAME-1);
				...
				if ( fatfs_entry_sfn_only(directoryEntry) )
				{
					...
					fat_strncpy(entry->filename, short_filename, FATFS_MAX_LONG_FILENAME-1);
