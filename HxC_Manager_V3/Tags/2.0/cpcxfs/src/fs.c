...
/*	trk = trk_calc (blk);
	...
	while (filled < dpb->BLS) 
	{
		...
		if (sector_ptr==NULL)
		{
			errorf(FALSE, "Warning: cannot find sector, unable to read data!");
			return NULL;
		}
...
/* fill track_header */
		...
		for (j=0;j<dpb->SECS;j++) {
			...
			if (h==0)
			{
				...
				trhd->sector[j].sector	= dpb->SEC1_side1 + sec_index;
			}
			else
			{
				trhd->sector[j].head	= dpb->side1_hd;	
				trhd->sector[j].sector	= dpb->SEC1_side2 + sec_index;
			}
...
/*		hd  = hd_calc(block);
	...
	for (k=0;k<secs;k++) {
		...
		if (hd==0)
		{
			sector = sec+dpb->SEC1_side1;
		}
		else
		{
			sector = sec+dpb->SEC1_side2;
		}
