...
//-----------------------------------------------------------------------------
 int _check_file_open(FL_FILE* file)
{
...
//-----------------------------------------------------------------------------
 int _open_directory(char *path, unsigned long *pathCluster)
{
	int levels;
	int sublevel;
...
//-----------------------------------------------------------------------------
 int _create_directory(char *path)
{
	FL_FILE* file; 
	FAT32_ShortEntry sfEntry;
...
//-----------------------------------------------------------------------------
 FL_FILE* _open_file(const char *path)
{
	FL_FILE* file; 
	FAT32_ShortEntry sfEntry;
...
//-----------------------------------------------------------------------------
 ...
 FL_FILE* _create_file(const char *filename)
{
	FL_FILE* file; 
	FAT32_ShortEntry sfEntry;
	char shortFilename[11];
...
int fl_fswrite(unsigned char * buffer, int size, void *f)
{
	...
	// Calculate start sector
	sector = 0;//file->bytenum / FAT_SECTOR_SIZE;
	cnt=size;
	while (cnt)
	{
                if (!fatfs_sector_writer(&Fs, file->startcluster, sector, buffer))
                {   	// Read failed - out of range (probably)
                   return -1;
                }
...
unsigned long fl_change_entry(UINT32 cluster,int dir,unsigned long entryindex,FAT32_ShortEntry *sfEntry,unsigned char * filename)
{
	...
	if (cluster != FAT32_INVALID_CLUSTER)
	{
          ...
          {
            case 1:
               ret=fatfs_next_entry(&Fs, cluster,entryindex,sfEntry,filename);
            break;
            case 0:
               ret=fatfs_prev_entry(&Fs, cluster,entryindex,sfEntry,filename);
            break;
            default:
            break;
          }
