...
unsigned int calc_ptr(unsigned int y_pos)
{
	unsigned int adr;

	adr=( ( (y_pos>>3) * 0x50) + (0x800 * (y_pos & 0x7)) );
...
char  buildincludefile(char *includefile,bmpinfo *info,unsigned char * dbuffer)
{
	...
	if(info->type!=0xff)
	{
		...
		fprintf(file2,"#endif\n");			
		fprintf(file2,"\n\n");
		fprintf(file2,"const unsigned char data_bmp%s[]={\n",temp);
	}
	else
	{
		fprintf(file2,"//////////////////////////////\n//\n//\n// Created by Binary2Header V0.5\n// (c) HxC2001\n// (c) PowerOfAsm\n//\n");
		...
		fprintf(file2,"#endif\n");			
		fprintf(file2,"\n\n");
		fprintf(file2,"const unsigned char data__%s[]={\n",temp);
...
char  buildbinfile(char *includefile,bmpinfo *info,unsigned char * dbuffer)
{
	int l,i,j,ptr;
	FILE * file2;
	...
	if(info->type!=0xff) sprintf(temp2,"data_bmp_%s.bin",temp);
	else sprintf(temp2,"data_%s.bin",temp);
	printf("Create %s :",temp2);


	file2=fopen(temp2,"wb");
/*	if(info->type!=0xff)
	{
		fprintf(file2,"//////////////////////////////\n//\n//\n// Created by Binary2Header V0.5\n// (c) HxC2001\n// (c) PowerOfAsm\n//\n");
		fprintf(file2,"// File: %s  Size: %d  (%d) x:%d y:%d\n//\n//\n",includefile,info->size,info->csize,info->xres,info->yres);
	...
	for(i=0;i<((info->size*8)/info->xres);i++)
	{
		for(j=0;j<(info->xres/8);j++)
		{
		 ptr=calc_ptr(i)+j;
		 if(ptr>=info->size)
		 {
		 ...
		 buffer[ptr]=dbuffer[(i*(info->xres/8))+j];
		}
	}
	
	fwrite(buffer,1,16384,file2);
	free(buffer);
		...
		do{
			...
			{//(info.size+100+1024
				...
				if(cbuffer!=NULL)
				{
					...
					buildbinfile(argv[1],&infoo,cbuffer);
					free(cbuffer);
					free(dbuffer);
				}
				else
